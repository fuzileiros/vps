...
def shell():
    ...
    printer('Iniciando Configuração do Speedtest.net....', quiet)
    try:
        speedtest = Speedtest(
    ...
    printer('Testando SERVIDOR %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('Somos NERDs Não Perdoamos Never...', quiet)
        try:
    ...
    if args.download:
        printer('Teste de velocidade de download', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(callback=callback)
        printer('Resultado de Download: %0.2f M%s/s' %
    ...
    if args.upload:
        printer('Teste de velocidade de upload', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.upload(callback=callback, pre_allocate=args.pre_allocate)
        printer('Resultado de Upload: %0.2f M%s/s' %
    ...
    if args.share and not machine_format:
        printer('Compartilhe os resultados: %s' % results.share())
